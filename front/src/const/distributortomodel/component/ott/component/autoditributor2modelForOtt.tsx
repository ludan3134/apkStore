...
import {grpcQueryBindDistributorModelListForOtt} from "../api/grpcQueryBindDistributorModelListForOtt";
import {authProxy} from "../../../../../app/auth/store/store";
import {TreeNode} from "primereact/treenode";
import {grpcBindDistributorModelListForOtt} from "../api/grpcBindDistributorModelListForOtt";
import {grpcAllModel} from "../../../../../app/distributor/api/grpcAllModel";

const Autodistributor2modelForOTT: React.FC<{ id: number }> = ({
                                                                   type,
                                                                   id,
                                                               }) => {
    ...
    const showModal = async () => {
        var res = await grpcQueryBindDistributorModelListForOtt(
            id,
            authProxy.token,
        );
    ...
    const onFinish = async (values: any) => {
        ...
        var res = await grpcBindDistributorModelListForOtt(
            id,
            authProxy.token,
            values.users,
        );
        if (res.status) {
            ...
            var res1 = await grpcQueryBindDistributorModelListForOtt(
                id,
                authProxy.token,
            );
            if (res1.status) {
...
export default Autodistributor2modelForOTT;
