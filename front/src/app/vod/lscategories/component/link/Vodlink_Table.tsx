...
import {VodLinkCalssStoreProxy} from "../../store/link/store";
import {grpcVodVodLinkList} from "../../api/link/grpcVodlinkList";
import {VodVodLink_Column} from "./Vodlink_Column";

export default function VodVodLink_Table() {
    // 刷新页面
    ...
    // 编辑版本信息
    const handleEditVodLink = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑点播视频链接",
        );
        if (foundOption) {
            VodLinkCalssStoreProxy.VodLinkCalssEdit = row;
    ...
    // 添加版本信息
    const handleAddVodLink = async () => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "新增点播视频链接",
        );
        if (foundOption) {
            goto(`${foundOption.url}/${foundOption.id}/${firstId}/${secondId}`);
            return true; // 添加返回 true 停止循环
    ...
    const handldeleteVodLink = async (row: VodLink) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "删除点播视频链接",
        );
        if (foundOption) {
            ...
            ids = [row.id];
            setDeleteRow(ids);
            setAction(ActionStore.DeleteVodLink);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            var res = await grpcVodVodLinkList(
                authProxy.token,
                parseInt(secondId),
                page,
                rowsPerPage,
    ...
    const handldeleteVodLinklist = async (selected) => {
        let foundOption = childrenMenu.find(
            ...
            <Table sx={{minWidth: 500}}>
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 30}} align="center">
                                    <Button
                                        variant="contained"
                                        size="small"
                                        onClick={() => handldeleteVodLink(row)}
                                    >
                                        删除链接
                                    </Button>
                                </StyledTableCell>
                ...
                <TableFooter>
                    <TableRow>
                        ...
                        {/*        variant="contained"*/}
                        {/*        size="small"*/}
                        {/*        onClick={() => handldeleteVodLinklist(selected)}*/}
                        {/*    >*/}
                        {/*        删除点播链接链接*/}
                        {/*    </Button>*/}
                        {/*</StyledTableCell>*/}
