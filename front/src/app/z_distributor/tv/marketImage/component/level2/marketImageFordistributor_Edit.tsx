...
import {grpcmarketImageEdit} from "../../api/level2/grpcmarketImageEdit";

export const MarketImageFordistributor_Edit = () => {
    // 获取编辑变量
    ...
    const onSubmit = async (add: AppBanner) => {
        ...
        try {
            ...
            // 执行数据更新操作
            var response = await grpcmarketImageEdit(add, authProxy.token);
            if (response.status) {
