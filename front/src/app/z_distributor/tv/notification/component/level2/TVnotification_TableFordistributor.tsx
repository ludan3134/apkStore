...
import {TVnotification_ColumnFordistributor} from "./TVnotification_ColumnFordistributor";
...
import {grpcTVNotificationList} from "../../../../../tv/TVnotification/api/grpcTVNotificationList";
import ConfirmDialog from "../../../../../../const/alert/confirmDialog";
import {StyledTableCell, StyledTableRow,} from "../../../../../../const/tablestyle";
import {useTranslation} from "react-i18next";
import {NotificationStoreProxy, useproxy_NotificationFilter,} from "../../store/level2/store";
import {Notification} from "../../../../../../api/tv_fs/v1/fm_pb";
import {useproxy_ProvideTemplatetableUrl} from "../../../deskImage/store/level1/store";
import ShowAgreementText from "../../../../../tsv/agreement/component/agreement_showcontent";

export default function TVnotification_TableFordistributor() {
    ...
    // 编辑apk
    const handleditnotification = async (row: Notification) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑电视消息通知",
        );
        if (foundOption) {
            NotificationStoreProxy.NotificationEdit = row;
    ...
    // 删除apk
    const handldeletenotification = async (row: Notification) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "删除电视消息通知",
        );
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            ...
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteTvnotification);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcTVNotificationList(
                page,
                rowsPerPage,
                authProxy.token,
                notificationFilter,
            );
            setRows(res.notificationList);
    ...
    return (
        <TableContainer component={Paper}>
            ...
            <Table sx={{minWidth: 500, marginTop: "10px", marginBottom: "10px"}}>
                <TVnotification_ColumnFordistributor
