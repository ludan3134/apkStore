...
import {CategoryVideo_Column} from "./CategoryVideo_Column";
...
import {RecommendStoreproxy} from "../../../recommendApk/store/level2/store";
import {grpcAllClassify} from "../../../../../tsv/classify/api/grpcAllClassify";

export default function CategoryVideo_Table() {
    // 刷新页面
    ...
    const handlfindsublevel = async (row) => {
        ...
        if (foundOption) {
            VideoStoreproxy.VideoFilter.categoryId = row.id;
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcAllClassify(videoFilter.classId, authProxy.token);
            RecommendStoreproxy.AllCategory = res.categoryList;
