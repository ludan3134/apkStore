...
import {ModelMarketImage_Column} from "./ModelMarketImage_Column";
import {StyledTableCell, StyledTableRow,} from "../../../../../../const/tablestyle";
import {AppBannerStoreProxy} from "../../store/level2/store";
import {grpcQueryProvideTemplatelist} from "../../api/level1/grpcQueryProvideTemplatelist";
import {grpcDeleteProvideTemplate} from "../../api/level1/grpcDeleteProvideTemplate";
import {ProvideTemplateStoreProxy} from "../../store/level1/store";

export default function ModelMarketImage_Table() {
    // 刷新页面
    ...
    const handlfindsublevel = async (row) => {
        ...
        if (foundOption) {
            AppBannerStoreProxy.AppBannerFilter.distributorId = row.distributorId;
            AppBannerStoreProxy.AppBannerFilter.modelId = row.modelId;
    ...
    const handleditProvideTemplate = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑盒子市场图片所属型号",
        );
        if (foundOption) {
                        ...
                        return (
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 25}} align="center">
                                    <span>{row.descriptionStore}</span>
                                </StyledTableCell>
