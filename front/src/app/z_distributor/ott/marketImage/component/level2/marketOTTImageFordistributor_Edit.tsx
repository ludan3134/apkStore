...
import {useproxy_AppBannerEdit, useproxy_AppBannertableUrl,} from "../../store/level2/store";
...
export const MarketOTTImageFordistributor_Edit = () => {
    // 获取编辑变量
    var backgroundEdit = useproxy_AppBannerEdit();
    // 获取addtable地址信息
    // 请求是否成功
    ...
    const onSubmit = async (add: AppBanner) => {
        ...
        try {
            ...
            // 执行数据更新操作
            var response = await grpcmarketOTTImageEdit(add, authProxy.token);
            if (response.status) {
