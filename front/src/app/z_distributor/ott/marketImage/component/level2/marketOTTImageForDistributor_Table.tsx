...
import {MarketOTTImageForDistributor_Column} from "./marketOTTImageForDistributor_Column";
...
import {grpcmarketOTTImageEdit} from "../../api/level2/grpcmarketOTTImageEdit";

export default function MarketOTTImageForDistributor_Table() {
    // 刷新页面
    ...
    const handleditBackground = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑盒子市场图片管理下级页面",
        );
        if (foundOption) {
    ...
    const handleAddBackground = async () => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "新增盒子市场图片管理下级页面",
        );
        if (foundOption) {
            IsUploadFileReset.IsReset = false;
    ...
    // 删除add
    const handldeleteBackground = async (row: AppBanner) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑盒子市场图片管理下级页面",
        );
        if (foundOption) {
            ...
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteOTTAppBanner);
            return true; // 添加返回 true 停止循环
        } else {
    ...
    const handleisShowOnMarket = async (row: AppBanner, checked: boolean) => {
    ...
    return (
        <TableContainer component={Paper}>
            ...
            <Table sx={{minWidth: 500, marginTop: "10px", marginBottom: "10px"}}>
                <MarketOTTImageForDistributor_Column
                        ...
                        return (
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 30}} align="center">
                                    <Image
                                        width={300}
                                        src={envUrls.ImgBaseUrl + `${row.url}`}
                                        preview={{scaleStep: 5}}
                                    />
                                </StyledTableCell>
                                <StyledTableCell style={{width: 30}} align="center">
