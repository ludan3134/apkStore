...
import {grpcDeskImageList} from "../api/grpcDeskImageList";
import {DeskImage_Column} from "./deskImage_Column";
import {DesktopImageStoreProxy, useproxy_DesktopImageFilter,} from "../store/store";
...
import {DeskImage_Filter} from "./deskImage_Filter";
import envUrls from "../../../../const/baseurl";

export default function DeskImage_Table() {
    // 刷新页面
    ...
    // 筛选条件
    var advertisementfilter = useproxy_DesktopImageFilter();
    // // 跳转路由
    ...
    const handldeleteBackground = async (row: HomeBackgroundImage) => {
        ...
        if (foundOption) {
            ...
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteDeskImage);
            return true; // 添加返回 true 停止循环
        } else {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcDeskImageList(
                page,
                rowsPerPage,
                authProxy.token,
                advertisementfilter,
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 50}} align="center">
                                    ...
                                    <img
                                        ...
                                        alt={"not found"} // 使用数据行中的 altText 属性作为 alt 文本
                                        // onError={(e) => {
                                        //     const img = e.currentTarget
                                        //     img.src = "/photo/apk_logo.png?w=164&h=164&fit=crop&auto=format&dpr=2 2x"
                                        // }}
