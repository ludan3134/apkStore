...
import {Lot_Column} from "./lot_Column";
import {LotStoreProxy} from "../store/store";
import {grpcLotList} from "../api/grpcLotList";
...
export default function Lot_Table() {
    // 刷新页面
    ...
    // 编辑apk
    const handleditlot = async (row: Lot) => {
        let foundOption = childrenMenu.find((option) => option.name === "编辑批次");
        if (foundOption) {
            LotStoreProxy.LotEdit = row;
            goto(`${foundOption.url}/${foundOption.id}`);
            return true; // 添加返回 true 停止循环
        } else {
            IsOpenDialog.IsOpen = true;
            IsOpenDialog.title = "暂无权限";
            IsOpenDialog.content = "暂时无编辑账户权限,请联系管理员修改!";
        }
    };
    //
    // const handldeletelotlist = async (selected) => {
    //     let foundOption = childrenMenu.find(option => option.name === "删除终端");
    //     if (foundOption) {
    //         IsConfirmDialog.IsOpen = true
    //         IsConfirmDialog.title = "确认删除选中lot列表"
    //         IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!"
    //         setDeleteRow(selected)
    //         setAction(ActionStore.Deletelot)
    //         return true; // 添加返回 true 停止循环
    //     } else {
    //         IsOpenDialog.IsOpen = true
    //         IsOpenDialog.title = "暂无权限"
    ...
    // 删除apk
    const handldeletelot = async (row: Lot) => {
        let foundOption = childrenMenu.find((option) => option.name === "删除消息");
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中lot";
            IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!";
            // const lotData: string[] = [row.id];
            // row. = true
            // setDeleteRow(row)
            // setAction(ActionStore.Deletelot)
            return true; // 添加返回 true 停止循环
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcLotList(page, rowsPerPage, authProxy.token);
            setRows(res.lotList);
                    ...
                    <TableRow>
                        <StyledTableCell style={{width: 30}} align="center">
                            <Stack spacing={2} direction={"row"}>
                                {/*<IconButton aria-label="delete" size="large"*/}
                                {/*            onClick={() => handldeletelotlist(selected)}>*/}
