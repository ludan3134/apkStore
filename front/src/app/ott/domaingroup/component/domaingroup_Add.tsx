...
import {grpcServerGroupInsert} from "../api/grpcdomaingroupInsert";
import {
    initialServerGroupParams,
    useproxy_ServerGroupEdit,
    useproxy_ServerGroupServerList,
    useproxy_ServerGrouptableUrl,
} from "../store/store";

export const Domaingroup_Add = () => {
    ...
    const formContext = useForm<ServerGroup>({
        // 初始化表单数据
        defaultValues: initialServerGroupParams,
    });
    ...
    const onSubmit = async (data: ServerGroup) => {
        ...
        try {
            // 执行数据更新操作
            var response = await grpcServerGroupInsert(data, authProxy.token);
            if (response.status) {
    ...
    return (
        ...
        >
            ...
            <Stack spacing={2}>
                <LocationBar location={"新增ServerGroup"}/>
                <TextFieldElement name="name" label="域名名称" required/>
