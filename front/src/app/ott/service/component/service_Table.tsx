...
export default function PortalInfo_Table() {
    // 刷新页面
    ...
    const handleditPortalInfo = async (row: PortalInfo) => {
        let foundOption = childrenMenu.find((option) => option.name === "编辑服务");
        if (foundOption) {
    ...
    const handldeletePortalInfo = async (row: PortalInfo) => {
        let foundOption = childrenMenu.find((option) => option.name === "删除服务");
        if (foundOption) {
