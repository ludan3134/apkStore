...
import {grpcFirewareEdit} from "../../api/fireware/grpcFirewareEdit";
import DialogActions from "@mui/material/DialogActions";

import {useproxy_FirewareEdit, useproxy_FirewaretableUrl,} from "../../store/fireware/store";
...
export const Fireware_Edit = () => {
    // 获取编辑变量
    var Fireware = useproxy_FirewareEdit();
    // 获取firewaretable地址信息
    ...
    const formContext = useForm<Firmware>({
        ...
        defaultValues: {
            id: Fireware.id,
            name: Fireware.name,
            modelId: Fireware.modelId,
            distributorId: Fireware.distributorId,
            isUse: Fireware.isUse,
        },
    });
    ...
    const onSubmit = async (data) => {
        setLoading(true);
        data.distributorId = Distributorvalue || Fireware.distributorId;
        data.modelId = Modelvalue || Fireware.modelId;
        // 如果其中一个文件存在，则执行更新操作
        try {
            // 执行数据更新操作
            var response = await grpcFirewareEdit(data, authProxy.token);
            if (response.status) {
    ...
    const handleResetForm = () => {
        ...
        setDistributorvalue(Fireware.distributorId);
        formContext.reset();
    };
    ...
    return (
        ...
        >
            ...
            <Stack spacing={2}>
                <LocationBar location={"编辑fireware"}/>
