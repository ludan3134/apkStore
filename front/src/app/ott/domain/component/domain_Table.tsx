...
import {ServerStoreProxy} from "../store/store";
...
import {grpcServerList} from "../api/grpcdomainList";
import {Domain_Column} from "./domain_Column";
import ConfirmDialog from "../../../../const/alert/confirmDialog";

export default function Domain_Table() {
    // 刷新页面
    ...
    // 编辑apk
    const handleditServer = async (row: Server) => {
        let foundOption = childrenMenu.find((option) => option.name === "编辑域名");
        if (foundOption) {
            ServerStoreProxy.ServerEdit = row;
    ...
    // 删除apk
    const handldeleteServer = async (row: Server) => {
        let foundOption = childrenMenu.find((option) => option.name === "删除域名");
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中Server";
            IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!";
            // const ServerData: string[] = [row.id];
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteServer);
            return true; // 添加返回 true 停止循环
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcServerList(page, rowsPerPage, authProxy.token);
            setRows(res.serverList);
                    ...
                    <TableRow>
                        <StyledTableCell style={{width: 30}} align="center">
                            <Stack spacing={2} direction={"row"}>
                                ...
                                {/*            onClick={() => handldeleteServerlist(selected)}>*/}
