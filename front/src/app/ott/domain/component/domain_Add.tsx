...
import {initialServerParams, useproxy_ServerEdit, useproxy_ServertableUrl,} from "../store/store";
...
import {grpcServerInsert} from "../api/grpcdomainInsert";
...
export const Domain_Add = () => {
    ...
    const formContext = useForm<Server>({
        // 初始化表单数据
        defaultValues: initialServerParams,
    });
    ...
    const onSubmit = async (data: Server) => {
        ...
        try {
            // 执行数据更新操作
            var response = await grpcServerInsert(data, authProxy.token);
            if (response.status) {
