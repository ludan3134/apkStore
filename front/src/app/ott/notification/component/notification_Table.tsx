...
import {grpcNotificationList} from "../api/grpcNotificationList";
import {Notification_Column} from "./notification_Column";
...
import Notification_Filter from "./notification_Filter";

export default function Notification_Table() {
    // 刷新页面
    ...
    const handleditnotification = async (row: Notification) => {
        let foundOption = childrenMenu.find((option) => option.name === "编辑消息");
        if (foundOption) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcNotificationList(page, rowsPerPage, authProxy.token, notification);
