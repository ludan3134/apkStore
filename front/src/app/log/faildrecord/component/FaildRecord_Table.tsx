...
import {grpcFaildRecordList} from "../api/grpcFaildRecordList";
...
import {FailedRecord_Column} from "./FailedRecord_Column";
import {useproxy_FailedRecordFilter} from "../store/failedRecord/store";
import FailRecord_Filter from "./faildRecord_Filter";

export default function FailedRecord_Table() {
    // 刷新页面
    ...
    // 筛选条件
    var FailedRecordfilter = useproxy_FailedRecordFilter();
    // // 跳转路由
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcFaildRecordList(page, rowsPerPage, FailedRecordfilter, authProxy.token);
            setRows(res.failedRecordList);
            setCount(res.pageMeta?.totalRecords);
        };
        fetchData(); // 调用异步函数
        IsConfirmDialog.refleshPage = false;
    }, [page, rowsPerPage, FailedRecordfilter, refleshPage]);
                    ...
                    {rows.map((row) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/*</StyledTableCell>*/}
                                {/*<StyledTableCell style={{width: 30}} align="center">*/}
                                {/*    /!*<IconButton aria-label="delete" size="large" onClick={() => handleditFailedRecord(row)}>*!/*/}
                                {/*    /!*    <EditIcon color={"primary"}/>*!/*/}
                                {/*    /!*</IconButton>*!/*/}
                                {/*    <Button variant="contained" size="small" onClick={() => handleditFailedRecord(row)}>*/}
                                {/*        编辑终端*/}
                                {/*    </Button>*/}
                                {/*</StyledTableCell>*/}
                                {/*<StyledTableCell style={{width: 30}} align="center">*/}
                                {/*    <IconButton aria-label="delete" size="large"*/}
                                {/*                onClick={() => handldeleteFailedRecord(row)}>*/}
