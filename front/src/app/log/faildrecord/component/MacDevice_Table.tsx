...
import {grpcQueryAllMacDeviceList} from "../api/grpcQueryAllMacDeviceList";
import {useproxy_MacDeviceFilter} from "../store/macdevice/store";
import {MacDevice_Column} from "./MacDevice_Column";
import {Timeformat} from "../../../../const/timeformat";
import MacDevice_Filter from "./MacDevice_Filter";

export default function MacDevice_Table() {
    // 刷新页面
    ...
    // 筛选条件
    var macDevice = useproxy_MacDeviceFilter();
    // // 跳转路由
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcQueryAllMacDeviceList(page, rowsPerPage, authProxy.token, macDevice);
            setRows(res.macDeviceList);
            setCount(res.pageMeta?.totalRecords);
        };
        fetchData(); // 调用异步函数
        IsConfirmDialog.refleshPage = false;
    }, [page, rowsPerPage, macDevice, refleshPage]);
