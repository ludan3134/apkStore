...
import {PricePlansStoreProxy} from "../store/store";
import {grpcpriceplansList} from "../api/grpcpriceplansList";
import {Priceplans_Column} from "./priceplans_Column";
import ConfirmDialog from "../../../../const/alert/confirmDialog";

export default function Priceplans_Table() {
    // 刷新页面
    ...
    const handleditBackground = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑价格套餐",
        );
        if (foundOption) {
            PricePlansStoreProxy.PricePlansEdit = row;
    ...
    const handldeleteBackground = async (row: HomeBackgroundImage) => {
        ...
        if (foundOption) {
            ...
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeletePricePlans);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcpriceplansList(page, rowsPerPage, authProxy.token);
            setRows(res.pricePlans);
