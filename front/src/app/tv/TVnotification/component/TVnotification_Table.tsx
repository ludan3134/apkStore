...
import {grpcTVNotificationList} from "../api/grpcTVNotificationList";
import {TVnotification_Column} from "./TVnotification_Column";
...
import TVNotification_Filter from "./TVnotification_Filter";
import {NotificationStoreProxy, useproxy_NotificationFilter} from "../store/store";

export default function TVnotification_Table() {
    // 刷新页面
    var refleshPage = useproxy_IsConfirmDialogRefleshPage();
    var notification = useproxy_NotificationFilter();
    // 获取本组件权限Id/下级权限
    ...
    const handldeletenotification = async (row: Notification) => {
        ...
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中notification";
            IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!";
            // const notificationData: string[] = [row.id];
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.TVDeletenotification);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    }, [page, rowsPerPage, refleshPage, notification]);
