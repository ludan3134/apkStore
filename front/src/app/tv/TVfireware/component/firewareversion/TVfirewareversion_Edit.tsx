...
import {useproxy_TVFirewaredetailEdit} from "../../store/firewareversion/store";
import {grpcTVFirewareversionEdit} from "../../api/firewareversion/grpcTVFirewareversionEdit";
...
export const TVFirewareversion_Edit = () => {
    const {id} = useParams();
    ...
    // 获取到对应版本数据
    var firmwareversion = useproxy_TVFirewaredetailEdit();
    ...
    const onSubmit = async (data: FirmwareDetail) => {
        setLoading(true);
        try {
            var response = await grpcTVFirewareversionEdit(data, authProxy.token);
            if (response.status) {
