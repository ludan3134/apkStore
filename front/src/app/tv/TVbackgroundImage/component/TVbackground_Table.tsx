...
import {TVBackgroundStoreProxy, useproxy_TVBackgroundFilter,} from "../store/store";
import {TVBackground_Column} from "./TVbackground_Column";
import {TVBackground_Filter} from "./TVbackground_Filter";
...
export default function TVBackground_Table() {
    // 刷新页面
    ...
    // // 筛选条件
    // var terminalfilter = useproxy_TVBackground1Filter();
    // // 跳转路由
    const navigate = useNavigate();
    // 筛选条件
    var TVBackgroundfilter = useproxy_TVBackgroundFilter();
    ...
    // 编辑add
    const handleditTVBackground = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑电视背景图片",
        );
        if (foundOption) {
            TVBackgroundStoreProxy.TVBackgroundlEdit = row;
    ...
    // 删除add
    const handldeleteTVBackground = async (row: HomeBackgroundImage) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "删除电视背景图片",
        );
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中TVBackground";
            IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!";
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteTVBackground);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    }, [page, rowsPerPage, refleshPage, TVBackgroundfilter]);
                        ...
                        return (
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 30}} align="center">
                                    ...
                                    {/*            onClick={() => handleditTVBackground(row)}>*/}
                                ...
                                <StyledTableCell style={{width: 30}} align="center">
                                    {/*<IconButton aria-label="delete" size="large"*/}
                                    {/*            onClick={() => handldeleteTVBackground(row)}>*/}
