...
import {TVLot_Column} from "./TVlot_Column";
...
import {TVLotStoreProxy} from "../store/store";

export default function TVLot_Table() {
    // 刷新页面
    ...
    // 编辑apk
    const handleditTVLot = async (row: Lot) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑电视批次",
        );
        if (foundOption) {
            TVLotStoreProxy.TVLotEdit = row;
    ...
    // 删除apk
    const handldeleteTVLot = async (row: Lot) => {
        let foundOption = childrenMenu.find((option) => option.name === "删除消息");
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中TVLot";
            IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!";
            // const TVLotData: string[] = [row.id];
            // row. = true
            // setDeleteRow(row)
            // setAction(ActionStore.DeleteTVLot)
            return true; // 添加返回 true 停止循环
                    ...
                    <TableRow>
                        <StyledTableCell style={{width: 30}} align="center">
                            <Stack spacing={2} direction={"row"}>
                                {/*<IconButton aria-label="delete" size="large"*/}
                                {/*            onClick={() => handldeleteTVLotlist(selected)}>*/}
