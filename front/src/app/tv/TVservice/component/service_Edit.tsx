...
import {useproxy_PortalInfoEdit, useproxy_PortalInfotableUrl,} from "../store/store";
...
import {grpcPortalInfoEdit} from "../api/grpcServiceEdit";

export const TVPortalInfo_Edit = () => {
    // 获取编辑变量
    ...
    const formContext = useForm<PortalInfo>({
        ...
        defaultValues: {
            id: PortalInfo.id,
            apkType: PortalInfo.apkType,
            apkVersion: PortalInfo.apkVersion,
            mwVersion: PortalInfo.mwVersion,
            provider: PortalInfo.provider,
            serviceType: PortalInfo.serviceType,
            serviceUrl: PortalInfo.serviceUrl,
            isService: PortalInfo.isService,
            createdAt: PortalInfo.createdAt,
            updatedAt: PortalInfo.updatedAt,
    ...
    const onSubmit = async (data: PortalInfo) => {
        ...
        try {
            // 执行数据更新操作
            var response = await grpcPortalInfoEdit(data, authProxy.token);
            if (response.status) {
    ...
    return (
        ...
        >
            ...
            <Stack spacing={2}>
                <LocationBar location={"编辑PortalInfo"}/>
