...
import {grpcsecondList} from "../../api/second/grpcsecondList";
import {Second_Column} from "./second_Column";
import {SubClass} from "../../../../../api/ks/v1/km_pb";
import CheckCircleIcon from "@mui/icons-material/CheckCircle";
import CancelIcon from "@mui/icons-material/Cancel";
import {SubClassStoreProxy} from "../../store/second/store";
...
export default function Second_Table() {
    ...
    const handleEditSubClass = async (row) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑二级分类",
        );
        if (foundOption) {
            SubClassStoreProxy.SubClassEdit = row;
    ...
    const handleAddSubCalss = async () => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "新增二级分类",
        );
        if (foundOption) {
            goto(`${foundOption.url}/${id}`);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    const handldeleteSubClass = async (row: SubClass) => {
        let foundOption = childrenMenu.find(
        ...
        if (foundOption) {
            ...
            ids = [row.id];
            setDeleteRow(ids);
            setAction(ActionStore.DeleteSubClass);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    };
    const handldeletesubclasslist = async (selected) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "删除二级分类",
        );
        if (foundOption) {
    ...
    // 查看三级分类
    const handlefindSubClass = async (row: SubClass) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "查看三级分类",
        );
        if (foundOption) {
            // console.log("foundOption",foundOption)
            goto(`${foundOption.url}/${foundOption.id}/${row.id}/${row.mainClassId}`);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            var res = await grpcsecondList(
                page,
                rowsPerPage,
                authProxy.token,
                parseInt(id),
            );
            setRows(res.subClassList);
                            ...
                            >
                                ...
                                <StyledTableCell align="center">
                                    <Button
                                        variant="contained"
                                        size="small"
                                        onClick={() => handleEditSubClass(row)}
                                    >
                                        编辑二级分类
                                    </Button>
                                </StyledTableCell>
                                <StyledTableCell align="center">
                                    <Button
                                        variant="contained"
                                        size="small"
                                        onClick={() => handldeleteSubClass(row)}
                                    >
                                        删除二级分类
                                    </Button>
                                </StyledTableCell>
                    ...
                    <TableRow>
                        <StyledTableCell>
                            <Button
                                variant="contained"
                                size="small"
                                onClick={() => handldeletesubclasslist(selected)}
                            >
                                删除二级分类
                            </Button>
                        </StyledTableCell>
