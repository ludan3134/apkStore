...
import {Xcmapping_Column} from "./xcmapping_Column";
import {useproxy_XStreamFilter, XStreamStoreProxy} from "../store/store";
import {XstreamResource} from "../../../../../api/ks/v1/km_pb";
import ConfirmDialog from "../../../../../const/alert/confirmDialog";
import {grpcXstreamList} from "../api/grpcXstreamList";
import {message} from "antd";
import {Timeformat} from "../../../../../const/timeformat";
import {Xcmapping_Filter} from "./xcmapping_Filter";

export default function Xcmapping_Table() {
    // 刷新页面
    var refleshPage = useproxy_IsConfirmDialogRefleshPage();
    // 获取本组件权限Id/下级权限
    var xStreamFilter = useproxy_XStreamFilter();
    ...
    // 编辑apk
    const handleditxstream = async (row: XstreamResource) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "编辑XC资源",
        );
        if (foundOption) {
            XStreamStoreProxy.XStreamEdit = row;
    ...
    // 删除apk
    const handldeletexstream = async (row: XstreamResource) => {
        let foundOption = childrenMenu.find(
            (option) => option.name === "删除XC资源",
        );
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            ...
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteTVMXStream);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcXstreamList(
                page,
                rowsPerPage,
                authProxy.token,
                xStreamFilter,
            );
            setRows(res.xstreamResourceList);
            setCount(res.pageMeta?.totalRecords);
        };
        fetchData(); // 调用异步函数
        IsConfirmDialog.refleshPage = false;
    }, [page, rowsPerPage, refleshPage, xStreamFilter]);
