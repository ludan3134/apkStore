...
import {useproxy_XStreamEdit, useproxy_XStreamtableUrl} from "../store/store";
import {XstreamResource} from "../../../../../api/ks/v1/km_pb";
import {grpcXstreamEdit} from "../api/grpcXstreamEdit";
...
export const Xcmapping_Edit = () => {
    // 获取编辑变量
    var xstream = useproxy_XStreamEdit();
    // 获取xstreamtable地址信息
    ...
    const formContext = useForm<XstreamResource>({
        ...
        defaultValues: {
            id: xstream.id,
            name: xstream.name,
            bouquetId: xstream.bouquetId,
            created: xstream.created,
            resourceType: xstream.resourceType,
            updated: xstream.updated,
            deleted: xstream.deleted,
        },
    });
    ...
    const onSubmit = async (data: XstreamResource) => {
        ...
        try {
            // 执行数据更新操作
            var response = await grpcXstreamEdit(data, authProxy.token);
            if (response.status) {
