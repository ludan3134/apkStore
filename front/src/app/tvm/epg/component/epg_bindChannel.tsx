...
import {grpcAllchannel} from "../api/grpcAllchannel";
import {grpcManualBindEpg} from "../api/grpcManualBindEpg";
...
export default function Epg_bindChannel({row}) {
    ...
    const handleSubFormSubmit = async (data) => {
        ...
        var epg = await grpcManualBindEpg(channelIds, name, authProxy.token);
        if (!epg.status) {
            message.error("绑定失败!");
            return;
        }
