import {ConnectError} from "@connectrpc/connect";
import {message} from "antd";
import {
    QueryAllAppInfoRequest,
    QueryAllAppInfoResponse,
    QueryMacAccountListRequest,
    QueryMacAccountListResponse,
} from "../../../../api/ks/v1/ks_pb";
import {PageMeta} from "../../../../api/com/v1/pagemeta_pb";
import {KsvClient} from "../../../../grpcClinet/grpcKsvClient";
import {MacAccount} from "../../../../api/ks/v1/km_pb";
import {authProxy} from "../../../auth/store/store";

export const grpcAppName = async (): Promise<QueryAllAppInfoResponse> => {
    const req = new QueryAllAppInfoRequest({});
    console.info("grpcTerminalList-req", req);
    var headers = new Headers();
    headers.set("token", authProxy.token);
    try {
        const res = await KsvClient.queryAllAppInfo(req, {headers: headers});
        console.info("grpcTerminalList-res", res);
        if (res === undefined) {
            return res;
        }
        if (res.status) {
            return res;
        }
    } catch (err) {
        if (err instanceof ConnectError) {
            message.error(err.message);
        }
        const er1 = ConnectError.from(err);
        console.log("error code", er1.code, "error message", er1.message);
    }
    return new QueryAllAppInfoResponse();
};
