...
import {grpcAgreementList} from "../api/grpcAgreementList";
import {Agreement_Column} from "./agreement_Column";
import {AgreementStoreproxy} from "../store/store";
...
import ShowAgreementText from "./agreement_showcontent";
...
export default function Agreement_Table() {
    // 刷新页面
    ...
    // 编辑apk
    const handleditAgreement = async (row: Agreement) => {
        let foundOption = childrenMenu.find((option) => option.name === "编辑协议");
        if (foundOption) {
            AgreementStoreproxy.AgreementEdit = row;
    ...
    // 删除apk
    const handldeleteAgreement = async (row: Agreement) => {
        let foundOption = childrenMenu.find((option) => option.name === "删除协议");
        if (foundOption) {
            IsConfirmDialog.IsOpen = true;
            IsConfirmDialog.title = "确认删除选中协议";
            row.deleted = true;
            setDeleteRow(row);
            setAction(ActionStore.DeleteAgreement);
            return true; // 添加返回 true 停止循环
        } else {
            message.error("暂无权限");
        }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcAgreementList(
                page,
                rowsPerPage,
                authProxy.token,
                distributorValue,
                    ...
                    {rows.map((row) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <StyledTableCell style={{width: 30}} align="center">
                                    <ShowAgreementText title={row.title} content={row.content}/>
                                </StyledTableCell>
                                <StyledTableCell style={{width: 30}} align="center">
                                    <Autodistributor2model type={1} id={row.id}/>
                                </StyledTableCell>
                                <StyledTableCell style={{width: 30}} align="center">
                                    <Button
                                        variant="contained"
                                        size="large"
                                        onClick={() => handleditAgreement(row)}
                                    >
                                        编辑
                                    </Button>
                                </StyledTableCell>
                                <StyledTableCell style={{width: 30}} align="center">
                                    <Button
                                        variant="contained"
                                        size="large"
                                        onClick={() => handldeleteAgreement(row)}
                                    >
                                        删除
                                    </Button>
                                </StyledTableCell>
