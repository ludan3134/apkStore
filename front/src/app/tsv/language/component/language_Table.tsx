...
import {grpcLanguageList} from "../api/grpcLanguageList";
import {Language_Column} from "./language_Column";
...
export default function Language_Table() {
    // 刷新页面
    ...
    // 编辑apk
    // const handleditRegion = async (row: Region) => {
    //     let foundOption = childrenMenu.find(option => option.name === "编辑地区语言");
    //     if (foundOption) {
    //         LanguageStoreproxy.LanguageEdit = row
    ...
    // 删除apk
    // const handldeleteLanguage = async (row: Region) => {
    //     let foundOption = childrenMenu.find(option => option.name === "删除地区语言");
    //     if (foundOption) {
    //         IsConfirmDialog.IsOpen = true
    //         IsConfirmDialog.title = "确认删除选中地区语言"
    //         IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!"
    //         var deleteIds = []
    //         deleteIds = [row.id]
    //         setDeleteRow(deleteIds)
    //         setAction(ActionStore.DeleteLanguage)
    //         return true; // 添加返回 true 停止循环
    //     } else {
    //         message.error("暂无权限")
    //     }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcLanguageList(page, rowsPerPage, authProxy.token);
            setRows(res.regionList);
                    ...
                    {rows.map((row) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/*            onClick={() => handleditRegion(row)}>*/}
                                {/*        编辑*/}
                                {/*    </Button>*/}
                                {/*</StyledTableCell>*/}
                                {/*<StyledTableCell style={{width: 30}} align="center">*/}
                                {/*    <Button variant="contained" size="large" onClick={() => handldeleteLanguage(row)}>*/}
                                {/*        删除*/}
                                {/*    </Button>*/}
                                {/*</StyledTableCell>*/}
                            </StyledTableRow>
