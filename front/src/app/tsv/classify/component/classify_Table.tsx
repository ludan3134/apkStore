...
import {grpcClassifyList} from "../api/grpcClassifyList";
...
import {Classify_Column} from "./classify_Column";

export default function Classify_Table() {
    // 刷新页面
    var refleshPage = useproxy_IsConfirmDialogRefleshPage();
    // 获取本组件权限Id/下级权限
    const {menuId, classId} = useParams();
    ...
    // 编辑apk
    // const handleditClassInfo = async (row: Category) => {
    //     let foundOption = childrenMenu.find(option => option.name === "编辑选项分类");
    //     if (foundOption) {
    ...
    //     let foundOption = childrenMenu.find(option => option.name === "删除选项分类");
    //     if (foundOption) {
    //         IsConfirmDialog.IsOpen = true
    //         IsConfirmDialog.title = "确认删除选中选项分类"
    //         IsConfirmDialog.content = "该操作无法撤销,请谨慎操作!"
    //         var deleteIds = []
    //         deleteIds = [row.id]
    //         setDeleteRow(deleteIds)
    //         setAction(ActionStore.DeleteClassify)
    //         return true; // 添加返回 true 停止循环
    //     } else {
    //         message.error("暂无权限")
    //     }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            var res = await grpcClassifyList(
                page,
                rowsPerPage,
                authProxy.token,
                Number(classId),
            );
            setRows(res.categoryList);
